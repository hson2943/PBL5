...
while True:
    ...
    try:
        for y1, x2, y2, x1 in facesInCurrentFrame:
            # create frame around face with 5 unit border away
    ...
    if fakeFlag == False:
        ...
        for encode, face in zip(encodesCurrentFrame, facesInCurrentFrame):
            ...
            # Nghe client
                ...
                cv2.putText(img, name, (x1+6, y2-6), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
                # mỗi lần gửi cách nhau 30s để khỏi spam
                if (currentTime - preTime).total_seconds() > 30:
                    preTime = currentTime
                             ...
                             <html>
                                   <body>
                                          <div style="width: 500px; height: 500px;background-color: orange; " class="unknow">
                                            ...
                                            <h1 style="margin-top: 10px;margin-left: 50px ; font-family: Arial, Helvetica, sans-serif;">Thời gian :""" + imgTime + """</h1>
                                          </div>
                                  </body>
                             </html>
                                    """
                    try:
                        msg = EmailMessage()
                        msg['Subject'] = 'Có người lạ lúc ' + imgTime
